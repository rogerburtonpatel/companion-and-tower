...
Notation L_lift X := ([X ⇒ X] * [X ⇒ X])%type.
...
Definition p_id {X : Type} {CL : CompleteLattice X} : L_lift X := (id, id).
...
Record evolution (f g : [X ⇒ X]) : Prop :=
  { evolve : ∀ R S, R ↣ₚ S → f R ↣ₚ g S }.

Record r_evolution (f g : [X ⇒ X]) : Prop :=
...
+ intros A g Hg; split; intros R S HRS.
  apply progress_limit_l';
    [ exact _ | intros f Af; destruct (Hg f Af) as [ Hf ]; now apply Hf ].
Qed.
...
Record p_evolution (f g : L_lift X) : Prop :=
...
split.
+ firstorder.
+ intros f g h [ H1 H2 ] [ Hs Hw ]; split.
  - rewrite <- H1. exact Hs.
  - rewrite <- H2. exact Hw.
+ intros A g Hg; split.
  - apply progress_limit_l';
      [ exact _ | intros f Af; destruct (Hg f Af) as [ Hs Hw ]; exact Hs ].
  - apply progress_limit_l';
      [ exact _ | intros f Af; destruct (Hg f Af) as [ Hs Hw ]; exact Hw ].
Qed.
