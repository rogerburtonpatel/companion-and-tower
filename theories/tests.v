...
Section s.
  ...
  Abort.
  (* a different [gfp] in a premise is fine: only the conclusion is checked *)
...
(** ** [accumulate] on hypotheses of the shape [f (elem R) u v]
    ...
    the candidate may sit under any monotone function at any nesting and under
    binders/conjunctions. *)
    (* todo: check disjunctions *)
...
Section accumulate_body.
  ...
  (** leaves need not apply the same function to the candidate *)
  ...
  (** splitting the conjunction into separate hypotheses *)
  Goal elem cb z y -> ba (elem cb) x y -> elem cb x y.
    intros h1 h2.
    accumulate acc.
  Abort.

  (** a hypothesis not mentioning the candidate is never reverted at all *)
  Goal elem cb z y -> ba (gfp b) x y -> elem cb x y.
    intros h1 h2.
    accumulate acc.
  Abort.
  ...
  (** but a premise may be, and the conclusion check must not reject it *)
  Goal ba (elem cb) z y -> elem cb x y.
    accumulate acc.
  Abort.

  (** conjunctions and quantifiers of [elem c] are accepted *)
  Goal elem cb z y -> elem cb x y /\ (forall n, elem cb n n).
    intro h.
    accumulate acc.
  Abort.
