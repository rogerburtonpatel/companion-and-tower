...
(* the hint database handles predicates built from the candidate.  the second
   branch is the older strategy, still needed for [Proper] obligations on
   INDUCTIVELY defined relation bodies (as in [experiments.v]), where the proof
   has to case on the premise rather than follow the syntax of a predicate. *)
...
Ltac monauto :=
  ...
  || solve [cbv; intros; induct_on_premise; try econstructor; try apply_leq; eauto]
